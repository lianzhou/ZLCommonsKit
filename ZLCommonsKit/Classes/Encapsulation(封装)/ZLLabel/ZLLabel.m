...
{
    static ZLLabel *label = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        label = [[ZLLabel alloc]init];
    });
    return label;
}
...
- (void)initializer
{
    ...
    self.textTapAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(zlLabel:didtapSelectStr:)]) {
            [weakSelf.delegate zlLabel:weakSelf didtapSelectStr:resultStr];
        }
    };
    //文本长按
    self.textLongPressAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
        NSString *resultStr = [text string];
        if (range.location >= text.length) return;
        if (self.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didLongPressSelectStr:)]) {
            [weakSelf.delegate zlLabel:weakSelf didLongPressSelectStr:resultStr];
        }
    };
    ...
    self.highlightTapAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
        ...
        if ([info objectForKey:@"at"]) {
            ...
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextTapSelectStr:withTapType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextTapSelectStr:url withTapType:ZLLabelLongPressAt];
            }
        }
        if ([info objectForKey:@"url"]) {
            NSString *url = info[@"url"];
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextTapSelectStr:withTapType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextTapSelectStr:url withTapType:ZLLabelLongPressUrl];
            }
        }
        if ([info objectForKey:@"phone"]) {
            NSString *url = info[@"phone"];
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextTapSelectStr:withTapType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextTapSelectStr:url withTapType:ZLLabelLongPressPhone];
            }
    ...
    self.highlightLongPressAction = ^(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
        ...
        if ([info objectForKey:@"at"]) {
            ...
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextLongPressSelectStr:withLongPressType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextLongPressSelectStr:url withLongPressType:ZLLabelLongPressAt];
            }
        }
        if ([info objectForKey:@"url"]) {
            NSString *url = info[@"url"];
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextLongPressSelectStr:withLongPressType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextLongPressSelectStr:url withLongPressType:ZLLabelLongPressUrl];
            }
        }
        if ([info objectForKey:@"phone"]) {
            NSString *url = info[@"phone"];
            if (weakSelf.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didHeightTextLongPressSelectStr:withLongPressType:)]) {
                [weakSelf.delegate zlLabel:weakSelf didHeightTextLongPressSelectStr:url withLongPressType:ZLLabelLongPressPhone];
            }
...
{
    if (ZLStringIsNull(emojiText)) {
        return CGSizeZero;
    }
    
    
    NSMutableAttributedString *resultText = [[NSMutableAttributedString alloc]initWithString:emojiText];
    [ZLLabel sharenInstance].font = font;
    resultText.font = font;
    //匹配表情
    resultText = [[ZLLabel sharenInstance] matchEmjo_withOldString:resultText];
    if ([ZLLabel sharenInstance].matchURL) {
        //匹配URL
        resultText = [[ZLLabel sharenInstance] matchURL_withOldString:resultText];
    }
    if ([ZLLabel sharenInstance].matchPhone) {
        resultText = [[ZLLabel sharenInstance] matchPhone_withOldString:resultText];
    }
...
- (void)addEndImageStr:(NSString *)imageStr with_maxNumberOfRows:(NSUInteger)maxNumberOfRows {
    ...
    [moreText setTextHighlightRange:NSMakeRange(0, moreText.length) color:[UIColor blueColor] backgroundColor:[UIColor redColor] tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didSpecialTextHeightTapSelectStr:)]) {
            [self.delegate zlLabel:self didSpecialTextHeightTapSelectStr:[text string]];
        }
    }];
...
- (NSMutableAttributedString *)matchEmjo_withOldString:(NSMutableAttributedString *)oldText{
    ...
    //获取到包含表情的数组
    NSArray *emjoRequest = [[ZLLabelHelper emojiRegexEmoticon] matchesInString:oldText.string options:kNilOptions range:oldText.rangeOfAll];
    ...
    for (NSTextCheckingResult *result in emjoRequest) {
        if (result.range.location == NSNotFound && result.range.length <=1) {
            ...
            NSString *imagePathName = [ZLLabelHelper emojiDictionary][emoString];
            NSString *imagePath = [@"ZLEmojiExpression.bundle" stringByAppendingPathComponent:imagePathName];
...
- (NSMutableAttributedString *)matchURL_withOldString:(NSMutableAttributedString *)oldText{
    ...
    NSArray *urlResults = [[ZLLabelHelper URLRegularExpression] matchesInString:oldText.string options:kNilOptions range:oldText.rangeOfAll];
...
- (NSMutableAttributedString *)matchPhone_withOldString:(NSMutableAttributedString *)oldText{
    ...
    highlightBorder.fillColor = UIColorRGBA(230, 230, 230, 0.5);
    // 匹配 手机号
    NSArray *phoneResults = [[ZLLabelHelper phoneNumerRegularExpression] matchesInString:oldText.string options:kNilOptions range:oldText.rangeOfAll];
...
- (NSMutableAttributedString *)matchAt_withOldString:(NSMutableAttributedString *)oldText{
    ...
    NSArray *atResults = [[ZLLabelHelper regexAt] matchesInString:oldText.string options:kNilOptions range:oldText.rangeOfAll];
        ...
        [matchAttribute setTextHighlightRange:NSMakeRange(currentLocation, currentLength)
                              ...
                              backgroundColor:nil
                                    tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
                                        @strongify(self);
                                        if (self.delegate && [self.delegate respondsToSelector:@selector(zlLabel:didSelectStrIndex:)]) {
                                            [self.delegate zlLabel:self didSelectStrIndex:i];
                                        }
                                    }];
        if (i != nameArray.count - 1) {
