...
@class JZAddShowPictureView;

//图片排版格式
typedef enum : NSUInteger {
    JZShowPicType_Default,//带添加按钮
    JZShowPicType_SpeedDial, //九宫格
    JZShowPicType_SpeedDial_ButOne,//一张特殊显示,其余九宫格显示
    JZShowPicType_StyleKindsOf, //不同图片数显示不同类型
} JZShowPicType;

@protocol JZAddShowPictureViewDelegate <NSObject>

@optional
//添加图片
- (void)addShowPictureView_addPictureEvent;
//删除图片
- (void)addShowPictureView:(JZAddShowPictureView *)pictureVirew deleteIndex:(NSInteger)deleteIndex;
//浏览图片
- (void)addShowPictureView:(JZAddShowPictureView *)pictureVirew browsePictureIndex:(NSInteger)selectedIndex withSuperImagesView:(UIView *)imagesView;

//添加长按手势
- (void)addShowPictureView:(JZAddShowPictureView *)pictureVirew longPress:(UILongPressGestureRecognizer *)press;

@end


@interface JZAddShowPictureView : UIView
@property(nonatomic,weak)id<JZAddShowPictureViewDelegate>pictureDelegate;
...
//图片排版类型
@property(nonatomic,assign)JZShowPicType picType;
//允许选择的最大图片数
