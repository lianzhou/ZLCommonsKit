...
@interface JZPicItemCell ()

@end

@implementation JZPicItemCell
...
@interface JZShowPicCollectionViewCell ()


@end

@implementation JZShowPicCollectionViewCell
...
- (void)collectionCelDataWithModel:(JZPicModel *)picModel withIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([picModel.imageUrl hasPrefix:@"http://dfs.img.jzexueyun.com/"]) {
    ...
    //NSLog(@"JZPicltemCell Scheme: %@", [url scheme]);
    
    if([[url scheme] isEqualToString:@"http"] || [[url scheme] isEqualToString:@"https"]){
        [self.mainImageView jz_setImageWithString:imageUrl placeholderType:JZPlaceholderImageTypeDefault_60_60];
    }
    else{
        @autoreleasepool {
...
- (UIImageView *)mainImageView {
    if (!_mainImageView) {
        ...
        [_mainImageView jz_setTarget:self action:@selector(imageTapClick:)];
