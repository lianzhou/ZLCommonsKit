...
//选中的数据
@property (nonatomic,strong) ZLIndexPath *selectIndexPath;
...
{
    ZLIndexPath *indexPath = [[self alloc] initWithColumn:column withRow:row];
    return indexPath;
}
...
- (void)setDataSource:(id<ZLPickerViewDataSource>)dataSource
{
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(pickerView:numberOfRowInColumn:)]) {
        if (component != 0) {
            return [_dataSource pickerView:self numberOfRowInColumn:[ZLIndexPath indexPathWithColumn:component withRow:0 withItem:_selectIndexPath.row]];
        }else{
            return [_dataSource pickerView:self numberOfRowInColumn:[ZLIndexPath indexPathWithColumn:component withRow:0 withItem:0]];
        }
    }
    return 0;
}
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(pickerView:dataForRowAtIndexPath:)]) {
        return [_dataSource pickerView:self dataForRowAtIndexPath:[ZLIndexPath indexPathWithColumn:component withRow:row withItem:0]];
    }
    return nil;
}
...
- (CGFloat)pickerView:(UIPickerView *)pickerView rowHeightForComponent:(NSInteger)component
{
    if (_delegate && [_delegate respondsToSelector:@selector(pickerView:heightForRowAtIndexPath:)]) {
        return [_delegate pickerView:self heightForRowAtIndexPath:[ZLIndexPath indexPathWithColumn:component withRow:0]];
    }
    return 35.0f;
}
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if(component < _numOfMenu - 1){
        if (_dataSource && [_dataSource respondsToSelector:@selector(pickerView:numberOfRowInColumn:)]) {
            NSInteger allRow = [_dataSource pickerView:self numberOfRowInColumn:[ZLIndexPath indexPathWithColumn:component withRow:0]];
            if (row >= allRow) {
                row = allRow - 1;
            }
        }
        _selectIndexPath = [ZLIndexPath indexPathWithColumn:component withRow:row];
    }
    ...
    if (btn == _sureBtn) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(pickerView:disSelectRowAtIndexPaths:)]) {
            for (int i = 0; i < _numOfMenu; i++) {
                ...
                ZLIndexPath *indexPath ;
                if (i > 0) {
                    ZLIndexPath *lastIndexPath = [_selectDataArray objectAtIndex:(i - 1)];
                    indexPath = [ZLIndexPath indexPathWithColumn:i withRow:row withItem:lastIndexPath.row];
                    if (_dataSource && [_dataSource respondsToSelector:@selector(pickerView:numberOfRowInColumn:)]) {
                        [_dataSource pickerView:self numberOfRowInColumn:indexPath];
                    }
                }else{
                    indexPath = [ZLIndexPath indexPathWithColumn:i withRow:row];
                }
