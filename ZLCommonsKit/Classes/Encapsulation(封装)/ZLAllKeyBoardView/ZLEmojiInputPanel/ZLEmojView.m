...
@interface ZLEmojView ()<UIScrollViewDelegate,ZLEmojiBottomViewdelagate,ZLEmojiBoardDelegate>
...
@property(nonatomic,strong)ZLEmojiBottomView *bottomView;
@property(nonatomic,strong)ZLEmojiBoardView *emojiBoardView;
...
- (void)initializer{
    ...
    self.emojiPageControl.numberOfPages = [ZLEmojiHelper emojiAllPage];
}
...
//        make.bottom.mas_equalTo(self.emojiPageControl.mas_top).priorityHigh();
        make.height.mas_equalTo(ZL_EmojiKeyBoardHeight - self.bottomView_Height);
    }];
    
}

#pragma mark -ZLEmojiBottomViewdelagate
...
#pragma mark - ZLEmojiBoardDelegate
...
- (void)pageChange:(UIPageControl *)sender {
    ...
    [self.emojiBoardView.allEmojiCollectionView scrollRectToVisible:CGRectMake(sender.currentPage *SCREEN_WIDTH, 0, SCREEN_WIDTH, ZL_EmojiKeyBoardHeight - self.bottomView_Height) animated:YES];
...
- (ZLEmojiBottomView *)bottomView
{
    if (!_bottomView) {
        _bottomView = [[ZLEmojiBottomView alloc]init];
...
- (ZLEmojiBoardView *)emojiBoardView {
    
    if (!_emojiBoardView) {
        _emojiBoardView = [[ZLEmojiBoardView alloc] init];
