...
@interface ZLEmojiBottomView ()<UIScrollViewDelegate>
...
@implementation ZLEmojiBottomView
...
- (void)initData
{
    ...
    ZLEmojiInfo *emojiInfo = [[ZLEmojiHelper shareInstance].emojList objectAtIndex:19];
     NSString *imagePath = [@"ZLEmojiExpression.bundle" stringByAppendingPathComponent:emojiInfo.imageName];
...
- (void)didSelectedItemIndex:(NSInteger)index
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(zlEmojiBottomView:didSelectedIndex:)]) {
        [self.delegate zlEmojiBottomView:self didSelectedIndex:index - 200];
    }
}
...
- (void)sendBtnAction
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(zlBottomViewdidSendAction)]) {
        [self.delegate zlBottomViewdidSendAction];
    }
}
#pragma mark - 懒加载
- (UIButton *)senderBtn
