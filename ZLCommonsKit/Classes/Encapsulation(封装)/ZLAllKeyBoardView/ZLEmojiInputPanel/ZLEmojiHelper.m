...
@implementation ZLEmojiHelper

+(ZLEmojiHelper *)shareInstance
{
    static ZLEmojiHelper *share;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        share = [[ZLEmojiHelper alloc]init];
...
//    NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"/emoji.plist"];
    ...
    for (int i = 0; i < emojiArray.count; i++) {
        ...
        ZLEmojiInfo *emojInfo = [[ZLEmojiInfo alloc]init];
...
+ (NSString *)emojiIconName:(NSString *)iconName {
    ...
    NSString *imagePath = [@"ZLEmojiExpression.bundle" stringByAppendingPathComponent:iconName];
    return imagePath;
}
...
+ (NSInteger)emojiCountCurrentPage {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (([ZLSystemUtils deviceScreenSize].width>=414)) {
            emojiCountOfPage = 23;
        }else{
            emojiCountOfPage = 20;
        }
