...
#import "ZLKeyBoardToolsPanelView.h"
...
#define ZLEMJOSENDER_HEIGHT 40.5
#define ZLHEADERHEIGHT 50
#define ZL_BTN_HEIGHT 40
#define ZL_INPUT_HEIGHT 36
...
@interface ZLKeyBoardView ()<UITextViewDelegate,ZLEmojViewDelegate,ZLAudionRecordManagerDelegate,ZLKeyBoardToolsPanelDelegate,HPGrowingTextViewDelegate,ZLShowContentViewDelegate>
{
...
@property(nonatomic,strong)ZLEmojView *emojiView;   //表情View
@property(nonatomic,strong)UILabel *inputNumberLabel;   //输入的字数
@property(nonatomic,strong)UIButton *audionPressBtn;    //按住说话按钮
@property(nonatomic,strong)ZLAudionRecordManager *audioManager; /* 录音组件 */
@property(nonatomic,strong)MBProgressHUD *hudTipView;   //提示View
@property(nonatomic,strong)ZLKeyBoardToolsPanelView *moreView;  //工具面板
@property(nonatomic,strong)UIButton *completeBtn;   //完成按钮
@property(nonatomic,assign)ZLKeyBoardViewType boardType;
@property(nonatomic,strong)ZLShowContentView *showContentView;  //输入内容view
...
- (void)initData
{
    self.backgroundColor = ZL_KMainColor;
...
- (void)initAudioRecord
{
    ...
    _audioManager = [ZLAudionRecordManager shareInstance];
...
#pragma mark - 约束
- (void)makeConstraints_withViewType:(ZLKeyBoardViewType)viewType{
    ...
    switch (viewType) {
        case ZLKeyBoardViewDefault:
        {
            
            [self.videoBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(self.headerView.mas_left).mas_offset(10);
                make.top.mas_equalTo(self.headerView).mas_offset((self.headerHeight - ZL_BTN_HEIGHT)/2);
                make.width.mas_offset(ZL_BTN_HEIGHT);
                make.height.mas_offset(ZL_BTN_HEIGHT);
            }];
            [self.inputTextView mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(self.videoBtn.mas_right).mas_offset(10);
                make.top.mas_equalTo(self.headerView.mas_top).mas_offset((self.headerHeight- ZL_INPUT_HEIGHT)/2);
                make.height.mas_offset(ZL_INPUT_HEIGHT);
                make.width.mas_offset(SCREEN_WIDTH - (10+ZL_INPUT_HEIGHT)*3- 10*2);
            }];
        ...
        case ZLKeyBoardViewEmoji_Input_More:
        {
            [self.emjioBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(self.headerView.mas_left).mas_offset(15);
                make.top.mas_equalTo(self.headerView.mas_top).mas_offset((self.headerHeight - ZL_BTN_HEIGHT)/2);
                make.width.mas_offset(ZL_BTN_HEIGHT);
                make.height.mas_offset(ZL_BTN_HEIGHT);
            }];
            [self.inputTextView mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.top.mas_equalTo(self.headerView.mas_top).mas_offset((self.headerHeight- ZL_INPUT_HEIGHT)/2);
                make.height.mas_offset(ZL_INPUT_HEIGHT);
                make.width.mas_offset(SCREEN_WIDTH - (15+ZL_INPUT_HEIGHT)*2- 15*2);
            }];
            [self.moreBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        case ZLKeyBoardViewInput_Emoji_More://输入框+表情+更多
        {
            ...
            [self.moreBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.right.mas_equalTo(self.headerView.mas_right);
                make.top.mas_equalTo(self.headerView.mas_top).mas_offset((self.headerHeight - ZL_BTN_HEIGHT)/2);
                make.width.mas_equalTo(ZL_BTN_HEIGHT);
                make.height.mas_equalTo(ZL_BTN_HEIGHT);
            }];
        ...
        case ZLKeyBoardViewPic_Carmera_Emoji_Send://图片+相机+表情
        {
        ...
        case ZLKeyBoardViewRecordEmoji_Send://输入框+表情
        {
            ...
            [self.inputTextView mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.right.mas_equalTo(self.emjioBtn.mas_left).mas_offset(-15);
                make.top.mas_equalTo(self.headerView.mas_top).mas_offset((self.headerHeight- ZL_INPUT_HEIGHT)/2);
                make.height.mas_equalTo(ZL_INPUT_HEIGHT);
            }];
            break;
        }
        case ZLKeyBoardViewRecordEmoji://表情
        {
            [self.showContentView mas_updateConstraints:^(MASConstraintMaker *make) {
    ...
    switch (sender.tag) {
        case ZLKeyBoardView_Emoji:
        {
            ...
            if (sender.selected) {
                ...
                _keyBoardHeight = ZL_EmojiKeyBoardHeight;
                ...
                if (![self.inputTextView resignFirstResponder]) {
                    ...
                    [UIView animateWithDuration:0.25 animations:^{
                        ...
                        [self mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(ZL_EmojiKeyBoardHeight+self.headerHeight+_showContentHeight+self.changeTextHeight);
                        ...
                        [_blackView mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(KHeight - (ZL_EmojiKeyBoardHeight+self.headerHeight+_showContentHeight+self.changeTextHeight));
                        }];
                    }];
                    if (self.delegate && [self.delegate respondsToSelector:@selector(keyBoardView:didChangeKeyBoardHeight:)]) {
                        [self.delegate keyBoardView:self didChangeKeyBoardHeight:ZL_EmojiKeyBoardHeight+self.headerHeight];
                    }
                }
            }else{
        ...
        case ZLKeyBoardView_Record:
        {
            ...
            if (sender.selected) {
                ...
                if (![self.inputTextView resignFirstResponder]) {
                    ...
                    [UIView animateWithDuration:0.25 animations:^{
                        [self mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(ZLHEADERHEIGHT);
                            make.bottom.mas_offset(0);
                        }];
                        [_showContentView mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(_showContentHeight);
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(keyBoardView:didChangeKeyBoardHeight:)]) {
                        [self.delegate keyBoardView:self didChangeKeyBoardHeight:ZLHEADERHEIGHT];
                    }
                }
            }else{
            ...
            if (sender.selected) {
                ...
                if (![self.inputTextView resignFirstResponder]) {
                    ...
                    [UIView animateWithDuration:0.25 animations:^{
                        _showContentHeight = _showPicHeight;
                        [self mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(ZL_MoreToolKeyBoardHeight+self.headerHeight+_showContentHeight+self.changeTextHeight);
                            make.bottom.mas_offset(0);
                        }];
                        [_showContentView mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(_showContentHeight);
                        ...
                        [_blackView mas_updateConstraints:^(MASConstraintMaker *make) {
                            make.height.mas_offset(KHeight - (ZL_MoreToolKeyBoardHeight+self.headerHeight+_showContentHeight+self.changeTextHeight));
                        }];
                    }];
                    if (self.delegate && [self.delegate respondsToSelector:@selector(keyBoardView:didChangeKeyBoardHeight:)]) {
                        [self.delegate keyBoardView:self didChangeKeyBoardHeight:ZL_MoreToolKeyBoardHeight+self.headerHeight];
                    }
                }
                _moreView.hidden = NO;
            }else{
        ...
        case ZLKeyBoardView_Pic:
        {
            [ZLSystemUtils assetsAuthorizationStatusAuthorized:^{
                if (self.delegate && [self.delegate respondsToSelector:@selector(keyBoardView:btnSelectedAction:)]) {
                    [self.delegate keyBoardView:self btnSelectedAction:ZLKeyBoardView_Pic];
                }
                ...
                NSString * message = [NSString stringWithFormat:@"请在iPhone的\"设置-隐私-照片\"选项中，\r允许\"%@\"访问你的手机相册",[ZLSystemUtils getTheAppName]];
                [ZLAlertHUD alertShowTitle:@"没有权限访问" message:message otherButtonTitle:@"确定" continueBlock:^{
        ...
        case ZLKeyBoardView_Camera:
        {
            [ZLSystemUtils videoAuthorizationStatusAuthorized:^{
                
                if (self.delegate && [self.delegate respondsToSelector:@selector(keyBoardView:btnSelectedAction:)]) {
                    [self.delegate keyBoardView:self btnSelectedAction:ZLKeyBoardView_Camera];
                }
                [self keyBoardViewHiden];
                [self.inputTextView resignFirstResponder];
                
            } restricted:^{
                NSString * message = [NSString stringWithFormat:@"请在iPhone的\"设置-隐私-相机\"选项中，\r允许\"%@\"访问你的相机",[ZLSystemUtils getTheAppName]];
                [ZLAlertHUD alertShowTitle:@"没有权限访问" message:message otherButtonTitle:@"确定" continueBlock:^{
                    [ZLSystemUtils openSystemSetting];
                }];
            }];
...
- (void)keyboardWillHide:(NSNotification *)notification
{
    ...
    if (_emjioBtn.selected) {
        ...
        keyHeight = ZL_EmojiKeyBoardHeight+self.headerHeight;
        ...
        keyHeight = ZL_MoreToolKeyBoardHeight+self.headerHeight;
    }else{
        _emojiView.hidden = YES;
...
- (BOOL)growingTextViewShouldReturn:(HPGrowingTextView *)growingTextView {
    ...
    [self mas_updateConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(ZLHEADERHEIGHT);
        make.bottom.mas_equalTo(self.mas_bottom);
    }];
    [self.inputTextView resignFirstResponder];
    return YES;
...
{
    //    _showContentHeight =_showPicHeight;
    //    self.changeTextHeight = height - ZL_INPUT_HEIGHT;
    //    [self.inputTextView mas_updateConstraints:^(MASConstraintMaker *make) {
    ...
    if (height == ZL_INPUT_HEIGHT) {
        if (self.boardType == ZLKeyBoardViewRecordEmoji_Send && self.isTopViewController) {
            _showContentHeight =_showPicHeight;
            self.changeTextHeight = height - ZL_INPUT_HEIGHT;
            [self.inputTextView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.mas_equalTo(height);
...
- (void)growingTextViewDidChange:(HPGrowingTextView *)growingTextView
{
    ...
    if (self.boardType == ZLKeyBoardViewPic_Carmera_Emoji_Send ||self.boardType == ZLKeyBoardViewRecordEmoji_Send) {
...
#pragma mark - ZLEmojViewDelegate

- (void)emojiViewDeleteClick:(ZLEmojView *)emojiView {
    
    
    if(ZLStringIsNull(self.inputTextView.text))
    {
        return;
    }else{
...
#pragma mark -ZLAudionRecordManagerDelegate
...
- (void)audioManager:(ZLAudionRecordManager *)manager didFinishRecord_WithRecordPath:(NSString *)recordPath withDuration:(CGFloat)duration
{
    ...
    ZLAudioModel *model = [[ZLAudioModel alloc]init];
...
#pragma mark -ZLKeyBoardToolsPanelViewDelegate
...
#pragma mark - ZLShowContentViewDelegate
...
- (UIButton *)emjioBtn
{
    if (_emjioBtn == nil) {
        ...
        _emjioBtn.tag = ZLKeyBoardView_Emoji;
...
- (UIButton *)videoBtn
{
    if (_videoBtn == nil) {
        ...
        _videoBtn.tag = ZLKeyBoardView_Record;
...
- (UIButton *)moreBtn
{
    if (_moreBtn == nil) {
        ...
        _moreBtn.tag = ZLKeyBoardView_More;
...
- (UIButton *)picBtn
{
    if (_picBtn == nil) {
        ...
        _picBtn.tag = ZLKeyBoardView_Pic;
...
- (UIButton *)cameraBtn {
    ...
    if (_cameraBtn == nil) {
        ...
        _cameraBtn.tag = ZLKeyBoardView_Camera;
...
- (ZLEmojView *)emojiView
{
    if (_emojiView == nil) {
        _emojiView = [[ZLEmojView alloc]init];
...
- (ZLKeyBoardToolsPanelView *)moreView
{
    if (_moreView == nil) {
        _moreView = [[ZLKeyBoardToolsPanelView alloc]initWithFrame:CGRectZero with_itemsArray:nil];
        _moreView.hidden = YES;
...
- (UIButton *)completeBtn
{
    if (_completeBtn == nil) {
        ...
        _completeBtn.tag = ZLKeyBoardView_Finish;
...
- (ZLShowContentView *)showContentView
{
    if (_showContentView == nil) {
        _showContentView = [[ZLShowContentView alloc]initWithFrame:CGRectZero];
