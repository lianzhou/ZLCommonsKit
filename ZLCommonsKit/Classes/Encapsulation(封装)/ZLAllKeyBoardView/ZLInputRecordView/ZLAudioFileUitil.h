...
@interface ZLAudioFileUitil : NSObject
/* 设置本地缓存路径 */
+ (void)setupAudioFileLocalStorePath:(ZLAudioModel*)audioFile;

/*设置本地缓存路径--和聊天不同不需要时间戳*/
+ (void)setupAudioFileLocalStorePathNoTime:(ZLAudioModel*)audioFile;

/*创建一个临时转码文件的缓存地址----和聊天不同不需要时间戳*/
+ (void)setupAudioFileTempEncodeFilePathNoTime:(ZLAudioModel*)audioFile;
...
// * 根据远程路径创建Wav格式本地路径
// */
//+ (void)createUserWAVStatisticsPathFolderWith:(ZLAudioModel *)audioModel;
///**
// * 根据远程路径创建amr格式本地路径
// */
//+ (void)createUserAMRStatisticsPathFolderWith:(ZLAudioModel *)audioModel;
@end
