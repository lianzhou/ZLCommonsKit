...
#import "ZLKeyBoarfToolCollectionCell.h"
#import <Masonry/Masonry.h>
#import "ZLSystemMacrocDefine.h"

#define ZLITOOLITEM_WIDTH (SCREEN_WIDTH-30)/4

static NSString *ZLKeyBoarfToolCollectionCellID = @"ZLKeyBoarfToolCollectionCell";
@interface ZLKeyBoardToolsPanelView()<UICollectionViewDelegate,UICollectionViewDataSource,UIScrollViewDelegate,UICollectionViewDelegateFlowLayout>
...
@implementation ZLKeyBoardToolsPanelView
...
- (void)createContentScrollViewWithArray:(NSArray *)itemArray
{
    ...
    [_contentScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(ZL_MoreToolKeyBoardHeight);
    }];
    ...
    _contentScrollView.contentSize = CGSizeMake(SCREEN_WIDTH *divisorNum, ZL_MoreToolKeyBoardHeight);
    ...
    for (int i = 0; i < divisorNum; i++) {
        ...
        [_itemMainCollectionView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.mas_offset(ZL_MoreToolKeyBoardHeight);
        }];
        ...
        [_itemMainCollectionView registerClass:[ZLKeyBoarfToolCollectionCell class] forCellWithReuseIdentifier:ZLKeyBoarfToolCollectionCellID];
        if (i == 0) {
    ...
    _mainPageControl.currentPageIndicatorTintColor = ZL_KMainColor;
    if (divisorNum >1 ) {
...
{
    ZLKeyBoarfToolCollectionCell *itemCell = [collectionView dequeueReusableCellWithReuseIdentifier:ZLKeyBoarfToolCollectionCellID forIndexPath:indexPath];
    ...
    itemCell.selectBlock = ^(){
        if(self.toolDelegate && [self.toolDelegate respondsToSelector:@selector(zlKeyBoardToolsPanel:didSelectedWithItem:withTitle:)]) {
            [self.toolDelegate zlKeyBoardToolsPanel:self didSelectedWithItem:curretnDict withTitle:[curretnDict objectForKey:@"iconTitle"]];
        }
    };
    return itemCell;
}
