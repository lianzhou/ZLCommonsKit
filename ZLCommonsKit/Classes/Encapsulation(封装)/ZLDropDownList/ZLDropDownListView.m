...
@interface ZLDropDownListView ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong) ZLDropDownConfig * dropDownConfig;

@property (nonatomic,assign) BOOL isAnimate;

@end

@implementation ZLDropDownListView
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView dropDownListViewShowType:(ZLDropDownListViewShowType)dropDownListViewShowType{
    ...
    if (dropDownListViewShowType == ZLDropDownListViewShowTypeNormal) {
        self.dropDownConfig.dropDownViewLeft = targetView.frame.origin.x + (targetView.frame.size.width/2);
    }else if (dropDownListViewShowType == ZLDropDownListViewShowTypeLeft){
        self.dropDownConfig.dropDownViewLeft = 0.1f;
    }else if (dropDownListViewShowType == ZLDropDownListViewShowTypeRight){
        self.dropDownConfig.dropDownViewLeft = -0.1f;
    }else if (dropDownListViewShowType == ZLDropDownListViewShowTypeCenter){
    ...
    for (ZLDropDownItem * dropDownItem in self.dropDownConfig.dropDownItems) {
...
{
     ZLDropDownItem * dropDownItem = [self.dropDownConfig.dropDownItems objectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ZLDropDownCell  *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [(ZLDropDownCell *)[cellClass alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
...
@implementation ZLDropDownConfig
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (NSMutableArray <ZLDropDownItem *>*)dropDownItems{
    if (!_dropDownItems) {
