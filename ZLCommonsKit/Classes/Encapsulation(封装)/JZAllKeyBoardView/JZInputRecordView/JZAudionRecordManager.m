...
@interface JZAudionRecordManager ()<AVAudioRecorderDelegate>
...
@implementation JZAudionRecordManager

+ (instancetype)shareInstance
{
    static JZAudionRecordManager *_manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _manager = [[JZAudionRecordManager alloc]init];
    });
    return _manager;
}
...
- (void)createRecord
{
    ...
    if (!self.recordSetting) {
        self.recordSetting = [JZAudioRecordSettings defaultQualitySetting];
    }
...
{
    NSString *cacheDirectory = [JZFilePath documentDirectoryPath:JZFilePathMainAudioCacheFile];
    NSString *fileName = [NSString stringWithFormat:@"%@.mp4",JZStringCurrentTimeStamp];
    NSString *localFilePath = [cacheDirectory stringByAppendingPathComponent:fileName];

    NSLog(@"本次语音路径:%@",localFilePath);
    if (![JZFilePath fileExist:cacheDirectory]) {
        
        [[JZFilePath fileManager] createDirectoryAtPath:cacheDirectory withIntermediateDirectories:YES attributes:nil error:nil];
    }
