...
@interface ZLTitleScreollView()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation ZLTitleScreollView
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(NSArray *)dataSourceArr isScrol:(BOOL)isScrol{
    ...
    if (self) {
        ...
        if ([model isKindOfClass:[ZLBaseTitleModel class]]) {
            ZLBaseTitleModel *baseModel = (ZLBaseTitleModel *)model;
            Class cellClass = [ZLBaseTitleModel classForContentType:baseModel.collectionCellModelType];
            [_titleCollectionView registerClass:cellClass forCellWithReuseIdentifier:baseModel.identifier];
        } else {
            [_titleCollectionView registerClass:[ZLTitleCollectionFirstCell class] forCellWithReuseIdentifier:@"ZLTitleCollectionFirstCell"];
        }
 ...
 *  动画类型
 */
- (void)setAnimationType:(ZLTitleAnimationType)animationType {
    _animationType = animationType;
    if (animationType == ZLTitleAnimationTypeColorAndLine || animationType == ZLTitleAnimationTypeOnlyLine) {
 ...
 *  下划线宽度类型
 */
- (void)setLineWidthType:(ZLTitleLineWidthType)lineWidthType {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([model isKindOfClass:[ZLBaseTitleModel class]]) {
        ...
        ZLBaseTitleModel *baseModel = (ZLBaseTitleModel *)model;
        ZLTitleCollectionBaseCell  *cell = (ZLTitleCollectionBaseCell *)[collectionView dequeueReusableCellWithReuseIdentifier:baseModel.identifier forIndexPath:indexPath];
        
        [cell settingModelData:model indexPath:indexPath];
        
        if (indexPath.item == _selectIndexPath.item) {
            if (_animationType == ZLTitleAnimationTypeOnlyLine) {
                if (_isFirst) {
                    _isFirst = NO;
                    //底部横线的frame
                    [self updateLineViewFrameWithCell:cell];
                }
            }else if (_animationType == ZLTitleAnimationTypeTransformScale) {
                //设置标题选中缩放比
                [cell setSelectCellTransform:_selectSclae];
            } else if (_animationType == ZLTitleAnimationTypeCustom) {
                //自定义动画
                if ([self.delegate respondsToSelector:@selector(ZLtitleScreollView:CollectionView:animationCellForRowAtIndexPath:)]) {
                    [self.delegate ZLtitleScreollView:self CollectionView:collectionView animationCellForRowAtIndexPath:indexPath];
                }
            }
        } else {
            //设置标题缩放比
        ...
        ZLTitleCollectionFirstCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZLTitleCollectionFirstCell" forIndexPath:indexPath];
        ...
        if (indexPath.item == _selectIndexPath.item) {
            ...
            if (_animationType == ZLTitleAnimationTypeTransformScale) {
                //设置选中缩放比
                 [cell setSelectCellTransform:_selectSclae];
                
            } else if(_animationType == ZLTitleAnimationTypeColorAndLine || _animationType == ZLTitleAnimationTypeOnlyLine){
                if (_isFirst) {
                    _isFirst = NO;
                    //底部横线的frame
                    [self updateLineViewFrameWithCell:cell];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_animationType == ZLTitleAnimationTypeColorAndLine || _animationType == ZLTitleAnimationTypeOnlyLine) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
    ...
    if ([self.delegate respondsToSelector:@selector(ZLtitleScreollView:CollectionView:didSelectItemAtIndexPath:)]) {
        [self.delegate ZLtitleScreollView:self CollectionView:collectionView didSelectItemAtIndexPath:indexPath];
    }
...
- (void)selectedItemAnimationIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(ZLtitleScreollView:CollectionView:didSelectItemAtIndexPath:)]) {
        [self.delegate ZLtitleScreollView:self CollectionView:_titleCollectionView didSelectItemAtIndexPath:indexPath];
    }
}
#pragma mark -- 联动方法
    ...
    if (_animationType == ZLTitleAnimationTypeTransformScale) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if ([model isKindOfClass:[ZLBaseTitleModel class]]) {
                ...
                for (ZLTitleCollectionBaseCell *cell in _titleCollectionView.visibleCells) {
                    //设置缩放比
                    [cell setNoSelectCellTransform:CGAffineTransformIdentity];
                }
                
                ZLTitleCollectionBaseCell * currentCell= (ZLTitleCollectionBaseCell*)[_titleCollectionView cellForItemAtIndexPath:_selectIndexPath];
                //设置选中缩放比
                [currentCell setSelectCellTransform:_selectSclae];
            } else {
                for (ZLTitleCollectionFirstCell * cell in _titleCollectionView.visibleCells) {
                    //标题颜色和大小
                    [cell setNoSelectFont:_titleFont color:_titleColor];
                    [cell setNoSelectCellTransform:CGAffineTransformIdentity];
                }
                ZLTitleCollectionFirstCell * currentCell= (ZLTitleCollectionFirstCell*)[_titleCollectionView cellForItemAtIndexPath:_selectIndexPath];
                //标题颜色和大小
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView isRight:(BOOL)isRight{
    ...
    if (_animationType == ZLTitleAnimationTypeTransformScale) {
        ZLTitleCollectionBaseCell *nextCcell = nil;
        ZLTitleCollectionBaseCell *currentCcell = nil;
        CGFloat slide ;
        CGFloat rightScale ;
        CGFloat leftScale;
        ...
        if (isRight) {//向右偏移
            ...
            if (index+1 != _dataSourceArr.count) {
                nextCcell= (ZLTitleCollectionBaseCell*)[_titleCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index+1 inSection:0]];
                currentCcell= (ZLTitleCollectionBaseCell*)[_titleCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
            }
        }else {//向左偏移
            ...
            nextCcell= (ZLTitleCollectionBaseCell*)[_titleCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:nextIndex inSection:0]];
            currentCcell= (ZLTitleCollectionBaseCell*)[_titleCollectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:currentIndex inSection:0]];
        }
        ...
        //标题颜色
        if (_titleColor) {
            orignClorArr = [ZLSystemUtils changeUIColorToRGB:_titleColor];
        } else {
            orignClorArr = [ZLSystemUtils changeUIColorToRGB:UIColorHex(0x333333)];
        }
        //标题选中颜色
        if (_titleSelectColor) {
            selectColorArr = [ZLSystemUtils changeUIColorToRGB:_titleSelectColor];
        } else {
            selectColorArr = [ZLSystemUtils changeUIColorToRGB:UIColorHex(0x999999)];
        }
        
        id  model = [_dataSourceArr objectAtIndex:index];
        if ([model isKindOfClass:[NSString class]]) {
            ZLTitleCollectionFirstCell *fCurrentCcell = (ZLTitleCollectionFirstCell *)currentCcell;
            ZLTitleCollectionFirstCell *fNextCcell = (ZLTitleCollectionFirstCell *)nextCcell;
