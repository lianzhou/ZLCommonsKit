...
@interface JZLocalVideoPlayer ()<NSURLSessionDownloadDelegate,NSURLSessionDataDelegate>
...
@implementation JZLocalVideoPlayer
...
didFinishDownloadingToURL:(NSURL *)location{
    ...
    NSString *movePath =  [[JZFilePath mainVideoCacheRemoteDirectory] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",[_videoUrlString md5String]]];
    ...
    if ([controller isKindOfClass:[JZAVPlayerViewController class]]) {
        //开启播放
        ...
        ALActionSheetView *sheetView=[[ALActionSheetView alloc] initWithTitle:nil cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@[@"保存视频"] handler:^(ALActionSheetView *actionSheetView, NSInteger buttonIndex) {
            if (buttonIndex == 0)
            {
                ...
                if ([_videoUrlString hasPrefix:@"http"] || [_videoUrlString hasPrefix:@"https"]) {
                    movePath =  [[JZFilePath mainVideoCacheRemoteDirectory] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",[_videoUrlString md5String]]];
                } else {
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo: (void *)contextInfo {
    if (error) {
        [JZAlertHUD showTipTitle:[NSString stringWithFormat:@"保存失败:%@",error.localizedDescription]];
    }else{
