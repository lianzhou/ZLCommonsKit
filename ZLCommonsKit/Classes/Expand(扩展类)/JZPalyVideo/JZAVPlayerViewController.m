...
@interface JZAVPlayerViewController ()<UIViewControllerTransitioningDelegate>

@property (nonatomic, strong) JZLocalVideoPlayer *player;
...
@implementation JZAVPlayerViewController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)viewDidLoad {
    ...
    if (!JZStringIsNull(_imageName)) {
        if([_urlString hasPrefix:@"http"] || [_urlString hasPrefix:@"https"]){
            [self.playerView jz_setImageWithURL:[NSURL URLWithString:_imageName] placeholderType:JZPlaceholderImageTypeNone];
        } else {
...
- (void)play{
    ...
    switch (internetStatus) {
        ...
        case YYReachabilityStatusWWAN:
        {
            if([_urlString hasPrefix:@"http"] || [_urlString hasPrefix:@"https"]){
                //缓存保存数据的路径
                NSString *movePath =  [[JZFilePath mainVideoCacheRemoteDirectory] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",[self.urlString md5String]]];

                if (![[NSFileManager defaultManager] fileExistsAtPath:movePath]) {
                    [JZAlertHUD alertShowTitle:nil message:@"当前网络为手机流量,是否继续" otherButtonTitle:@"继续" continueBlock:^{
                        [weakSelf startToPlay:weakSelf.playButton];
                    }];
...
- (void)startToPlay:(UIButton *)btn {
    ...
    //注意url不能为空
    if (!JZStringIsNull(url.absoluteString)) {
        [[JZLocalVideoPlayer sharedInstance] playWithUrl:url showView:self.playerView backView:self.view size:_videoSize];
        WEAKSELF
        [JZLocalVideoPlayer sharedInstance].downloadSuccessBlock = ^(){
...
- (void)goToBack {
    ...
    [[JZLocalVideoPlayer sharedInstance] stop];
...
- (void)_onPresent {
    ...
    CGSize backViewSize =  [JZUtilsTools imageSizeWithMaxSize:CGSizeMake(SCREEN_WIDTH, SCREEN_HEIGHT) minSize:CGSizeMake(40, 40) originalSize:self.videoSize];
