...
#import "ZLRecordVideoViewController.h"
...
@interface ZLRecordVideoViewController ()
{
...
@implementation ZLRecordVideoViewController
...
//     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(backBtnClick) name:UIApplicationWillEnterForegroundNotification object:nil];
    ...
    [ZLSystemUtils videoAuthorizationStatusAuthorized:^{
        ...
        [ZLSystemUtils soundAuthorizationStatusAuthorized:^{
...
- (void)backBtnClick{
    ...
    if ([self.delegate respondsToSelector:@selector(recordVideoControllerDidCancel:completion:)]) {
        ...
        if ([controller isKindOfClass:[ZLRecordVideoViewController class]]) {
            [self releasePlayObject];
...
-(void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer
{
        ...
        
    {
        [ZLSystemUtils videoAuthorizationStatusAuthorized:^{
            ...
            if ([[ZLSystemUtils deviceString] isEqualToString:@"Simulator"]) {
                [ZLAlertHUD showTipTitle:@"警告：该设备是模拟器，并没有摄像头！"];
                return ;
            }
            ...
            [ZLSystemUtils soundAuthorizationStatusAuthorized:^{
                ...
                [ZLAlertHUD alertShowTitle:@"提示信息" message:@"提示信息" cancelButtonTitle:@"返回" otherButtonTitle:@"确定" continueBlock:^{
                    [self backBtnClick];
                } cancelBlock:^{
                    [self backBtnClick];
                }];
//                [ZLAlertHUD alertShowTitle:@"提示信息" message:MSG_SOUND_NO_AUTH cancelButtonTitle:@"确定" otherButtonTitles:@"" continueBlock:^{
//                    [self backBtnClick];
//                } cancelBlock:^{
//                    [self backBtnClick];
//                }];
...
//            [ZLAlertHUD alertShowTitle:@"提示信息" message:MSG_VIDEO_NO_AUTH cancelButtonTitle:@"确定" otherButtonTitles:@"" continueBlock:^{
//                 [self backBtnClick];
//            } cancelBlock:^{
//                [self backBtnClick];
//            }];
        ...
        
    {
        [ZLSystemUtils videoAuthorizationStatusAuthorized:^{
            ...
            [ZLSystemUtils soundAuthorizationStatusAuthorized:^{
                ...
                if (_count < 1) {
                    [ZLAlertHUD showTipTitle:@"小视频录制时间太短"];
                    _count = 0;
                    //移除中心录像view和图层layer
...
//    NSString *directoryPath = [NSString stringWithFormat:@"%@/%@/recordVideo/",document,ZL_App.userId];
    NSString *directoryPath = [ZLFilePath mainAudioCacheDirectory];
...
-(void)saveToPhotosAlbum:(NSString *)path
{
    if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(path)) {
        [ZLAlertHUD showTipTitle:@"正在处理..." toView:self.view];
        WEAKSELF
...
//            [[NSNotificationCenter defaultCenter] postNotificationName:ZL_RECORD_VIDEO object:self userInfo:@{@"videoPath":_pathToMovie,@"timeLength":@(_timeLength),@"videoSize":NSStringFromCGSize(_videoSize)}];
            [self releasePlayObject];
