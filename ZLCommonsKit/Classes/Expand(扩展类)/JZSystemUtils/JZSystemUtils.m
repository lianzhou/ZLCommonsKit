...
@implementation JZSystemUtils
//"@xx "后面的‘ ’,是特殊字符跟空格不一样
NSString *const JZ_IM_REMIND = @" ";
...
+ (NSString *)getMyDeviceAllInfo
{
    ...
    [deviceAllInfo appendString:[JZSystemUtils deviceString]];
    [deviceAllInfo appendString:@","];
    [deviceAllInfo appendString:[JZSystemUtils getAppVersion]];
...
{    
    ...
    return  [JZStringUitil stringIsNull:deviceString]?@"未知":deviceString;
...
+ (BOOL)wakePhoneCallWithNumber:(NSString *)number {
    ...
    if (JZStringIsMobilePhone(number) || JZStringIsPhone(number)) {
        
      return [JZSystemUtils openSystemSettingWithUrlString:[NSString stringWithFormat:@"tel://%@",number]];
    }
...
+ (BOOL)wakeSendMessage:(NSString *)phone {
    ...
    if (JZStringIsMobilePhone(phone) || JZStringIsPhone(phone)) {
        
       return [JZSystemUtils openSystemSettingWithUrlString:[NSString stringWithFormat:@"sms://%@",phone]];
    }
...
+ (BOOL)openThirdLibraryWithUrlString:(NSString *)urlString {
    ...
    if (JZStringIsNull(urlString)) {
        
        NSLog(@"链接为空");
        return NO;
    }
    
    return [JZSystemUtils openSystemSettingWithUrlString:urlString];
}
...
+ (BOOL)iPhone4Device
{
    return CGSizeEqualToSize((CGSize){320,480}, [JZSystemUtils deviceScreenSize]);
}

+ (BOOL)iPhone5Device
{
    return CGSizeEqualToSize((CGSize){320,568}, [JZSystemUtils deviceScreenSize]);
    
}

+ (BOOL)iPhone6Device
{
    return CGSizeEqualToSize((CGSize){375,667}, [JZSystemUtils deviceScreenSize]);
}

+ (BOOL)iPhone6PlusDevice
{
    return CGSizeEqualToSize((CGSize){414,736}, [JZSystemUtils deviceScreenSize]);
}

+ (BOOL)iPhoneXDevice {
    
    return CGSizeEqualToSize((CGSize){375,812}, [JZSystemUtils deviceScreenSize]);
}
...
{
    return [JZSystemUtils colorFromRed:red green:green blue:blue withAlpha:1.0];
}
...
{
    if(JZCheckObjectNull(value)||JZCheckObjectNull(key)){
        return YES;
    }else{
        return NO;
    }
